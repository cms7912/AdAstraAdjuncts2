// via: CloudKitSyncMonitor, version 1.1.1, 2022-01-10
...
public class SyncMonitor: ObservableObject {
   ...
   public init(setupState: SyncState = .notStarted,
      ...
      if #available(iOS 14.0, macCatalyst 14.0, *) { // Crashes on 13.7 w/o this, even though we have @available
				NotificationCenter.default.publisher(for: CoreData.NSPersistentCloudKitContainer.eventChangedNotification)
            .sink(receiveValue: { notification in
							if let cloudEvent = notification.userInfo?[CoreData.NSPersistentCloudKitContainer.eventNotificationUserInfoKey]
										as? CoreData.NSPersistentCloudKitContainer.Event {
   ...
   internal struct SyncEvent {
		 var type: CoreData.NSPersistentCloudKitContainer.EventType
      var startDate: Date?
      var endDate: Date?
      var succeeded: Bool
      var error: Error?
      ...
      /// Creates a SyncEvent from explicitly provided values (for testing)
		 init(type: CoreData.NSPersistentCloudKitContainer.EventType, startDate: Date?, endDate: Date?, succeeded: Bool,
           error: Error?) {
         self.type = type
      ...
      /// Creates a SyncEvent from an NSPersistentCloudKitContainer Event
		 init(from cloudKitEvent: CoreData.NSPersistentCloudKitContainer.Event) {
